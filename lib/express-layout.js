...
function viewRoot(view) {
  if(!Array.isArray(view.root)){
    ...
    for (i;i<l;i++) {
      var root = roots[i];
      var file = path.basename(view.path);
      var loc = path.resolve(root, file);
      var dir = path.dirname(loc);
      var fullPath = view.resolve(root,file);
      if(fullPath){
        return root;
      }
    }
  ...
  return function expressLayout (req, res, next) {
    ...
    res.render = function (view, options, fn) {
      ...
      if (!layout) {
        ...
        layout = path.relative(viewRoot(resolvedView), layoutView.path);
      }
